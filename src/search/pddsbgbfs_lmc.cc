...
vector<int> PDDSBGBFSLmc::operator()() {
...
void PDDSBGBFSLmc::SendTermination() {
...
bool PDDSBGBFSLmc::ReceiveTermination() {
  int has_received = 0;
...
void PDDSBGBFSLmc::ReceiveNodes() {
  MPI_Status status;
  int has_received = 0;
...
void PDDSBGBFSLmc::BytesToNode(const unsigned char *d) {
  int info[4];
  // info[0] = h
  // info[1] = operator
  // info[2] = parent
  ...
  vec_.InsertToAnother(info[3], parent_rank_);
  ...
  size_t offset = 4 * sizeof(int) + block_size_ * sizeof(uint32_t);
  memcpy(accepted, d + offset, ac_bytes_ * sizeof(uint8_t));

  open_.Push(h, node);
}

void PDDSBGBFSLmc::NodeToBytes(int to_rank, int a, int parent,
  ...
  size_t offset = 3 * sizeof(int) + block_size_ * sizeof(uint32_t);
  memcpy(outgo_buffer_[to_rank].data() + size + offset,
         accepted, ac_bytes_ * sizeof(uint8_t));
}

void PDDSBGBFSLmc::SendDD() {
  for (int i=0; i<world_size_; ++i) {
    if (i == rank_ || outgo_buffer_[i].empty()) continue;
    unsigned char *d = outgo_buffer_[i].data();
  ...
  while (has_received) {
    ...
    for (int i=0; i<n_nodes; ++i) {
      ...
      memcpy(tmp_packed_.data(), d + 3 * sizeof(int),
             block_size_ * sizeof(uint32_t));
      int node = -1;
      if (BytesDD(info, &node, tmp_packed_.data())) continue;
      ...
      if (h < h_min_) {
        ...
        size_t offset = 4 * sizeof(int) + block_size_ * sizeof(uint32_t);
        memcpy(outgo_buffer_[source].data() + size + offset, accepted,
               ac_bytes_ * sizeof(uint8_t));
      }
    }
...
bool PDDSBGBFSLmc::BytesDD(int *info, int* node, const uint32_t *state) {
  ...
  if (!result) {
    ...
    // info[1] = parent
    // info[2] = parent rank
    *node = vec_.GenerateNode(info[0], info[1], state);
...
int PDDSBGBFSLmc::Init() {
  ...
  if (rank_ == 0) {
    ...
    while (open_.Size() < world_size_) {
      ...
      uint8_t *p_accepted = vec_.GetBits(ac_bytes_, current_node, lm_vec_);
      ...
      for (auto o : applicable) {
        ...
        uint8_t *c_accepted = vec_.GetBits(ac_bytes_, child_node, lm_vec_);
        int h = lmc_(tmp_child_, domain_, p_accepted, c_accepted);
        ++evaluated;
        if (h < h_min_) h_min_ = h;
    ...
    if (goal_node == -1) {
      ...
      while (!distributed && !open_.IsEmpty()) {
        ...
        const uint8_t *p_accepted = vec_.GetBits(ac_bytes_, current_node,
                                                 lm_vec_);
        ...
        for (auto o : applicable) {
          ...
          NodeToBytes(i, o, current_node, tmp_packed_.data(), p_accepted);
...
vector<int> PDDSBGBFSLmc::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
