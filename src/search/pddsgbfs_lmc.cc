...
vector<int> PDDSGBFSLmc::operator()() {
...
void PDDSGBFSLmc::SendTermination() {
...
bool PDDSGBFSLmc::ReceiveTermination() {
  int has_received = 0;
...
void PDDSGBFSLmc::BytesToNode(const unsigned char *d) {
  int info[3];
  // info[0] = operator
  // info[1] = parent
  // info[2] = parent rank
...
void PDDSGBFSLmc::NodeToBytes(int to_rank, int a, int parent,
...
void PDDSGBFSLmc::ReceiveDD() {
  ...
  while (has_received) {
    ...
    for (int i=0; i<n_nodes; ++i) {
      ...
      memcpy(tmp_packed_.data(), d + 3 * sizeof(int),
             block_size_ * sizeof(uint32_t));
      if (BytesDD(d, &node, tmp_packed_.data())) continue;
      ...
      if (steal) {
        ...
        outgo_buffer_[source].resize(size + node_size_);
        memcpy(outgo_buffer_[source].data() + size, d, node_size_);
      }
    }
...
bool PDDSGBFSLmc::BytesDD(const unsigned char *d, int *node, uint32_t *state) {
  ...
  if (!result) {
    ...
    *node = vec_.GenerateNode(info[0], info[1], tmp_packed_.data());
    vec_.InsertToAnother(info[2], parent_rank_);
    vec_.GenerateBits(ac_bytes_, lm_vec_);
    ++generated;
...
vector<int> PDDSGBFSLmc::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
