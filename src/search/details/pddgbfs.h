...
template<class H>
void PDDGBFS<H>::SendTermination() {
...
template<class H>
bool PDDGBFS<H>::ReceiveTermination() {
  int has_received = 0;
...
template<class H>
void PDDGBFS<H>::BytesToNode(const unsigned char *d) {
  int info[3];
  // info[0] = operator
  // info[1] = parent
  // info[2] = parent rank
...
template<class H>
void PDDGBFS<H>::NodeToBytes(int to_rank, int a, int parent,
...
template<class H>
bool PDDGBFS<H>::BytesDD(const unsigned char *d) {
  memcpy(tmp_packed_.data(), d + 3 * sizeof(int),
         block_size_ * sizeof(uint32_t));
...
template<class H>
vector<int> PDDGBFS<H>::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
