...
template<class H>
vector<int> SWAstar<H>::operator()(int weight, const Domain &domain) {
  ...
  vector<int> step(size, -1);

  int goal = Search(weight, domain, table, packer, vec, step);

  return ExtractPath(vec, goal);
}

template<class H>
int SWAstar<H>::Search(int weight, const Domain &domain, const TrieTable &table,
  ...
  while (!open.IsEmpty()) {
    ...
    int child_step = step[current_node] + 1;
    ...
    for (auto o : applicable) {
      ...
      vec.InsertToAnother(child_step, step);
      ++generated;
...
template<class H>
vector<int> SWAstar<H>::ExtractPath(const NodeVector &vec, int node) {
