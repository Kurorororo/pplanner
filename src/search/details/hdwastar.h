...
template<class H>
vector<int> HDWAstar<H>::operator()() {
...
template<class H>
void HDWAstar<H>::SendTermination() {
...
template<class H>
bool HDWAstar<H>::ReceiveTermination() {
  int has_received = 0;
...
template<class H>
void HDWAstar<H>::BytesToNode(const unsigned char *d) {
  int info[4];
  // info[0] = operator
  // info[1] = parent
  // info[2] = parent rank
  ...
  vec_.InsertToAnother(info[3], g_values_);
  ++generated;
...
template<class H>
void HDWAstar<H>::NodeToBytes(int to_rank, int a, int parent, int g,
...
int HDWAstar<H>::Search() {
  ...
  if (rank_ == initial_rank) {
    ...
    vec_.InsertToAnother(0, g_values_);
  ...
  while (!ReceiveTermination()) {
    ...
    int parent_g = g_values_[current_node];
    ...
    for (auto o : applicable) {
      ...
      if (to_rank == rank_) {
        ...
        NodeToBytes(to_rank, o, current_node, g, tmp_packed_.data());
      }
    }
...
template<class H>
vector<int> HDWAstar<H>::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
