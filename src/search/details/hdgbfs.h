...
template<class H>
bool HDGBFS<H>::ReceiveTermination() {
  int has_received = 0;
...
template<class H>
void HDGBFS<H>::BytesToNode(const unsigned char *d) {
  int info[3];
  // info[0] = operator
  // info[1] = parent
  // info[2] = parent rank
...
template<class H>
void HDGBFS<H>::NodeToBytes(int to_rank, int a, int parent,
...
template<class H>
vector<int> HDGBFS<H>::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
