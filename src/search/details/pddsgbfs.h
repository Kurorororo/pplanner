...
template<class H>
vector<int> PDDSGBFS<H>::operator()() {
...
template<class H>
void PDDSGBFS<H>::SendTermination() {
...
template<class H>
bool PDDSGBFS<H>::ReceiveTermination() {
  int has_received = 0;
...
template<class H>
void PDDSGBFS<H>::BytesToNode(const unsigned char *d) {
  int info[3];
  // info[0] = operator
  // info[1] = parent
  // info[2] = parent rank
...
template<class H>
void PDDSGBFS<H>::NodeToBytes(int to_rank, int a, int parent,
  ...
  while (has_received) {
    ...
    for (int i=0; i<n_nodes; ++i) {
      ...
      if (steal) {
        ...
        memcpy(outgo_buffer_[source].data() + size, d,
...
template<class H>
bool PDDSGBFS<H>::BytesDD(const unsigned char *d) {
  memcpy(tmp_packed_.data(), d + 3 * sizeof(int),
         block_size_ * sizeof(uint32_t));
  bool result = closed_.Contain(vec_, tmp_packed_.data());
...
template<class H>
vector<int> PDDSGBFS<H>::ExtractPath(int node) {
  int sum_expanded = 0;
  int sum_evaluated = 0;
  int sum_generated = 0;
  int sum_deadend = 0;
