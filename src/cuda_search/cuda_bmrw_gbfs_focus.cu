...
void CudaBMRWGBFSFocus::Init(const boost::property_tree::ptree &pt) {
  ...
  rw_open_ = OpenListFactory(open_option_, evaluators);
...
bool CudaBMRWGBFSFocus::PopStates(vector<int> &parents) {
  int offset = 0;
  int i = 0;
...
void CudaBMRWGBFSFocus::GenerateChildren(int parent, vector<int> &values,
...
int CudaBMRWGBFSFocus::PushStates(const vector<int> &parents, vector<int> &arg_h) {
  ...
  for (auto i : arg_h) {
    ...
    if (h < best_h_) {
      ...
      opens_.push_back(std::move(OpenListFactory(open_option_, evaluators_)));
      opens_.back()->Push(values, node, true);
...
void CudaBMRWGBFSFocus::Restart() {
...
void CudaBMRWGBFSFocus::InitialEvaluate() {
...
int CudaBMRWGBFSFocus::GreedyOpen() {
  int arg_min = -1;

  //auto iter = std::remove_if(opens_.begin(), opens_.end(),
  //                           [](std::unique_ptr<OpenList> p)->bool
  //                           { return p->IsEmpty(); });
...
int CudaBMRWGBFSFocus::CpuExpand() {
...
vector<int> CudaBMRWGBFSFocus::Plan() {
  std::vector<int> parents(n_threads_);
  std::vector<int> arg_h(n_threads_);
  InitialEvaluate();

  while (true) {
    if (GreedyOpen() == -1 && rw_open_->IsEmpty()) Restart();
...
std::vector<int> CudaBMRWGBFSFocus::ExtractPlan(
    int node) {
...
void CudaBMRWGBFSFocus::DumpStatistics() const {
...
CudaBMRWGBFSFocus::~CudaBMRWGBFSFocus() {
