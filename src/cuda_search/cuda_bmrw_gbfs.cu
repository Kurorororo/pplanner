...
void CudaBMRWGBFS::Init(const boost::property_tree::ptree &pt) {
  ...
  graph_->ReserveByRAMSize(gbfs_ram);
  std::size_t size = rw_graph_->ReserveByRAMSize(rw_ram);
...
void CudaBMRWGBFS::RWInitialEvaluate() {
...
void CudaBMRWGBFS::InitialEvaluate() {
  auto state = problem_->initial();
...
vector<int> CudaBMRWGBFS::Plan() {
  ...
  while (true) {
    ...
    while(cudaEventQuery(fin) == cudaErrorNotReady){
      ...
      if (goal != -1) return ExtractPlan(goal, graph_);
    }

    int goal = PushStates(parents, arg_h);

    if (goal != -1) return ExtractPlan(goal, rw_graph_);
  }
}

std::vector<int> CudaBMRWGBFS::ExtractPlan(
    int node,
  ...
  while (graph->Parent(node) != -1) {
    if (graph->Action(node) == -1) {
      result.insert(result.begin(), sequences_[node].begin(),
                    sequences_[node].end());
    } else {
      result.insert(result.begin(), graph->Action(node));
    }
...
void CudaBMRWGBFS::DumpStatistics() const {
