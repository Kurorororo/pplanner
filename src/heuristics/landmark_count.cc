...
int LandmarkCount::ReachedSize(const vector<int> &state,
...
int LandmarkCount::NeededSize(const vector<int> &state,
...
int LandmarkCount::Evaluate(const vector<int> &state, int node) {
  ...
  if (node == 0) {
    ...
    const uint8_t *parent_accepted = search_graph_->Landmark(parent);
