...
std::unique_ptr<OpenList> OpenListFactory(
  ...
  if (alternating)
    return std::unique_ptr<AlternatingOpenList>(
        new AlternatingOpenList(tie_breaking, evaluators));
...
std::shared_ptr<OpenList> SharedOpenListFactory(
  ...
  if (alternating)
    return std::make_shared<AlternatingOpenList>(tie_breaking, evaluators);
