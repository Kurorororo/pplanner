#ifndef KPGBFS_DUMP_H_
#define KPGBFS_DUMP_H_
...
class KPGBFSDump : public Search {
  struct SearchNodeWithFlag : public SearchNodeWithNext {
    int id;
  };

 public:
  KPGBFSDump(std::shared_ptr<const SASPlus> problem,
 ...
 private:
  ...
  void IncrementID(std::shared_ptr<SearchNodeWithFlag> node);
  ...
  std::mutex open_mtx_;
  std::mutex stat_mtx_;
  std::vector<std::shared_ptr<SearchNodeWithFlag> > node_pool_;
};

}  // namespace pplanner

#endif  // KPGBFS_DUMP_H_
