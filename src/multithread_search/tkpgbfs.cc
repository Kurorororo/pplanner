...
std::shared_ptr<SearchNodeWithNext> TKPGBFS::LockedPop() {
  ...
  if (c > 0 && h > h_c_.load()) return nullptr;

  if (c == 0 || (c > 0 && h < h_c_.load())) h_c_.store(h);
...
void TKPGBFS::LockedPush(const vector<int>& values,
                         shared_ptr<SearchNodeWithNext> node,
                         bool is_preferred) {
  std::lock_guard<std::mutex> lock(open_mtx_);
...
void TKPGBFS::Expand(int i) {
  ...
  while (goal_ == nullptr) {
    ...
    for (auto o : applicable) {
      ...
      // LockedPush(values, child_node, is_preferred);
