...
void GBFSIndependent::InitHeuristics(int i,
...
void GBFSIndependent::Init(const boost::property_tree::ptree& pt) {
  goal_ = nullptr;
  ...
  for (int i = 0; i < n_threads_; ++i) {
    open_lists_.push_back(
        OpenListFactory<std::vector<int>, std::shared_ptr<SearchNode> >(
            open_list_option));
  }
...
void GBFSIndependent::InitialEvaluate() {
...
int GBFSIndependent::Evaluate(int i, const vector<int>& state,
...
std::shared_ptr<SearchNode> GBFSIndependent::GenerateSeeds() {
  ...
  while (open_lists_[0]->size() < n_threads_) {
    ...
    if (closed_lists_[0]->IsClosed(node->hash, node->packed_state)) continue;

    packer_->Unpack(node->packed_state.data(), state);
    closed_lists_[0]->Close(node);
    ++expanded_;
...
void GBFSIndependent::Expand(int i) {
  vector<int> state(problem_->n_variables());
  vector<int> child(problem_->n_variables());
  vector<int> applicable;
  unordered_set<int> preferred;
...
std::shared_ptr<SearchNode> GBFSIndependent::Search() {
  InitialEvaluate();
...
void GBFSIndependent::DumpStatistics() const {
