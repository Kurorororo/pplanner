...
class MultiFocusGBFS : public Search {
 public:
  using Focus = std::shared_ptr<OpenList<
    std::shared_ptr<SearchNodeWithHash> > >;
  ...
  void LockedPushFocus(Focus focus) {
    std::lock_guard<std::mutex> lock(open_mtx_);
  ...
  void CreateNewFocus(std::vector<int> &values,
    ...
    auto focus = SharedOpenListFactory<std::shared_ptr<SearchNodeWithHash> >(
        open_list_option_);
    focus->Push(values, node, is_pref);
 ...
 private:
  ...
  std::unique_ptr<OpenList<Focus> > foci_;
  std::mutex open_mtx_;
  std::mutex stat_mtx_;
