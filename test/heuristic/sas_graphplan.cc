...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    std::cerr << "Usage: sas_graphplan <filename>" << std::endl;
    exit(1);
  }
  ...
  auto result = Graphplan(initial, domain, schema, &graph, preferred);
  int result_cost = GraphplanCost(initial, domain, schema, &graph, preferred);
