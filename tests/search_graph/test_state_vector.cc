...
class StateVectorTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ...
    vector_0_ = std::make_shared<StateVector>(sas_0);
    ...
    vector_1_ = std::make_shared<StateVector>(sas_1);
    ...
    vector_2_ = std::make_shared<StateVector>(sas_0, 2);
  }

  std::shared_ptr<StateVector> vector_0_;
  std::shared_ptr<StateVector> vector_1_;
  std::shared_ptr<StateVector> vector_2_;
...
TEST_F(StateVectorTest, GetClosedWorks) {
  ...
  EXPECT_EQ(-1, vector_2_->GetClosed(state_0_));
  i = vector_2_->Add(state_0_);
  vector_2_->Close(i);
  EXPECT_EQ(i, vector_2_->GetClosed(state_0_));
  tmp_state_0 = state_0_;
  tmp_state_0[1] = 1;
  EXPECT_EQ(-1, vector_2_->GetClosed(tmp_state_0));
  i = vector_2_->Add(tmp_state_0);
  vector_2_->Close(i);
  EXPECT_EQ(i, vector_2_->GetClosed(tmp_state_0));
  tmp_state_0[2] = 1;
  EXPECT_EQ(-1, vector_2_->GetClosed(tmp_state_0));
  i = vector_2_->Add(tmp_state_0);
  vector_2_->Close(i);
  EXPECT_EQ(0, vector_2_->GetClosed(state_0_));
  EXPECT_EQ(i, vector_2_->GetClosed(tmp_state_0));
  tmp_state_0 = state_0_;
  tmp_state_0[1] = 1;
  EXPECT_EQ(1, vector_2_->GetClosed(tmp_state_0));
...
TEST_F(StateVectorTest, ExpandWorks) {
  int i = vector_0_->Add(state_0_);
  std::vector<int> tmp_state_0(state_0_.size());
  EXPECT_TRUE(vector_0_->Expand(i, tmp_state_0));
  EXPECT_EQ(state_0_, tmp_state_0);
  EXPECT_FALSE(vector_0_->Expand(i, tmp_state_0));
  state_0_[1] = 1;
  i = vector_0_->Add(state_0_);
  EXPECT_TRUE(vector_0_->Expand(i, tmp_state_0));
  EXPECT_EQ(state_0_, tmp_state_0);
  ...
  EXPECT_TRUE(vector_1_->Expand(i, tmp_state_1));
  EXPECT_EQ(state_1_, tmp_state_1);
  EXPECT_FALSE(vector_1_->Expand(i, tmp_state_1));
  state_1_[1] = 1;
  i = vector_1_->Add(state_1_);
  EXPECT_TRUE(vector_1_->Expand(i, tmp_state_1));
  EXPECT_EQ(state_1_, tmp_state_1);
