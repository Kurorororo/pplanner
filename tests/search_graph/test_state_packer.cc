...
class StatePackerTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ...
    packer_0_ = std::make_shared<StatePacker>(sas_0);
    ...
    packer_1_ = std::make_shared<StatePacker>(sas_1);

    for (auto range : ranges)
      state_1_.push_back(rand() % range);
  }

  std::shared_ptr<StatePacker> packer_0_;
  std::shared_ptr<StatePacker> packer_1_;
